...
function enabledApp(facilitator = makeFacilitator(), options = {}) {
  return createApp(loadConfig({ NODE_ENV: 'test', PAYMENTS_ENABLED: 'true' }), { facilitator, ...options });
}
...
test('duplicate nonce is aborted by the ledger before a second facilitator /verify call', async () => {
  let verifyCalls = 0;
  ...
  const ledger = createNonceLedger(':memory:');
  const app = enabledApp(facilitator, { nonceLedger: ledger });

  const payload = await signedPayload(await challenge(app));
  const { from, nonce } = payload.payload.authorization;

  const first = await paidRequest(app, payload);
  assert.equal(first.status, 200);
  assert.equal(verifyCalls, 1);
  assert.equal(ledger.getStatus(payload.accepted.network, from, nonce), 'settled');

  const second = await paidRequest(app, payload);
  assertNoProtectedData(second);
  assert.equal(verifyCalls, 1, 'facilitator.verify must not be called again for a duplicate nonce');
  assert.equal(decodeHeader(second.headers['payment-required']).error, 'duplicate_authorization_nonce');

  ledger.close();
});

test('duplicate nonce rejected by the ledger records a DUPLICATE lifecycle event distinct from a facilitator-detected one', async () => {
  let verifyCalls = 0;
  const facilitator = makeFacilitator({
    verify: async () => {
      verifyCalls += 1;
  ...
  assert.equal((await paidRequest(app, payload)).status, 200);
  const second = await paidRequest(app, payload);
  assertNoProtectedData(second);
  assert.equal(verifyCalls, 1, 'the facilitator must not be called again for the rejected duplicate');

  const events = app.locals.lifecycle.store.list();
  const duplicateEvents = events.filter(event => event.lifecycle_event_type === 'DUPLICATE');
  assert.equal(duplicateEvents.length, 1, 'exactly one DUPLICATE event -- caught by the ledger, never reached the facilitator');
  const [duplicateEvent] = duplicateEvents;
  assert.equal(duplicateEvent.status, 'DETECTED');
  assert.equal(duplicateEvent.evidence_confidence, 'VERIFIED');
  assert.equal(duplicateEvent.evidence_source, 'kitchen:nonce-ledger');
  assert.equal(duplicateEvent.evidence_pointer, 'kitchen:duplicate_authorization_nonce');
  assert.notEqual(duplicateEvent.evidence_source, 'facilitator:verify');

  ledger.close();
});

test('a nonce that fails verification can still be identified as failed, not stuck pending', async () => {
  const facilitator = makeFacilitator({
    verify: async () => ({ isValid: false, invalidReason: 'facilitator_rejected' })
  });
  const ledger = createNonceLedger(':memory:');
  ...
  assertNoProtectedData(await paidRequest(app, payload));
  assert.equal(ledger.getStatus(payload.accepted.network, from, nonce), 'failed');
